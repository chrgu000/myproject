...
//		getJedis();
	 ...
	 * 通过一致性哈希算法决定把数据存到哪台上，算是一种客户端负载均衡，所以添加是用这个（Redis 3.0之后支持服务端负载均衡）
    ...
    { 
        ...
        config.setMaxTotal(Integer.valueOf(bundle.getString("redis.pool.maxActiv")));
        //最大空闲连接数, 默认8个
    ...
    public static Jedis initialPool(String url, int port) 
    { 
    	//加载属性文件：redis.properties
    	ResourceBundle bundle = ResourceBundle.getBundle("redis");
    	// 池基本配置 
	...
	public static Jedis getJedis(){
		//连接本地的 Redis 服务
		jedis = new Jedis("192.168.182.141", 6379);
		System.out.println("Connection to server sucessfully");
		//设置 redis 字符串数据  需要启动redis
		jedis.auth("met12ED");//当redis设置密码后，需要先验证密码，才能进行下一步操作
